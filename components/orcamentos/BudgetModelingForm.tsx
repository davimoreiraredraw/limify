...
import OrcamentoModelingItemSidebar, {
  OrcamentoModelingItem,
} from "./OrcamentoModelingItemSidebar";
...
interface BudgetItem {
  ...
  total: number;
  exibir: boolean;
  isEditingField?: "tempoDesenvolvimento" | "metrosQuadrados" | null;
}
...
export default function BudgetModelingForm({
  ...
  const handleSaveItem = (item: OrcamentoModelingItem) => {
  ...
  const updateItemValue = (
    itemId: string,
    field: "tempoDesenvolvimento" | "metrosQuadrados",
    value: number
  ) => {
    setBudgetItems((prevItems) =>
      prevItems.map((item) => {
        if (item.id === itemId) {
          ...
          // Recalcular total baseado na complexidade e metros quadrados
          let total = 0;
          if (updatedItem.grauComplexidade === "sem") {
            total = updatedItem.metrosQuadrados * 150;
          } else if (updatedItem.grauComplexidade === "baixa") {
            total = updatedItem.metrosQuadrados * 200;
          } else if (updatedItem.grauComplexidade === "media") {
            total = updatedItem.metrosQuadrados * 300;
          } else {
            total = updatedItem.metrosQuadrados * 200;
          }
  ...
  const handleSubmitBudget = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/budgets/modeling", {
        method: "POST",
              ...
              {budgetItems.length === 0 ? (
                ...
                budgetItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      {item.isEditingField === "metrosQuadrados" ? (
                        <div className="relative">
                          <input
                            type="number"
                            className="pr-8 pl-4 py-2 w-full border border-gray-200 rounded-md bg-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 text-right"
                            placeholder="0"
                            value={item.metrosQuadrados || ""}
                            onChange={(e) => {
                              const value = parseFloat(e.target.value) || 0;
                              updateItemValue(
                                item.id,
                        ...
                        <div
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            startInlineEdit(item.id, "metrosQuadrados");
                          }}
                        >
                          {item.metrosQuadrados} m²
                        </div>
                      )}
                    </div>
                    <div className="text-right">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border flex gap-4 items-center">
                ...
                <div>
                  ...
                  <div className="text-xl font-bold mb-1">
                    Orçamento por Modelagem
                  </div>
                  <div className="text-gray-400 text-sm">
                    Orçamento baseado em metros quadrados e complexidade
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border flex gap-4 items-center">
                ...
                <div>
                  ...
                  <div className="text-gray-400 text-sm">
                    {tipoAmbiente === "interior"
                      ? "Projeto de interiores com modelagem"
                      : tipoAmbiente === "exterior"
                      ? "Projeto de exteriores com modelagem"
        ...
        initialItem={
          editingItem
            ? {
                ...
                metrosQuadrados: editingItem.metrosQuadrados,
