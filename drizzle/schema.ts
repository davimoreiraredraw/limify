import { pgTable, pgEnum, uuid, text, timestamp, integer, foreignKey, numeric, boolean, varchar, index, unique, serial } from "drizzle-orm/pg-core"
...
export const planType = pgEnum("plan_type", ['free', 'basic', 'expert', 'business'])
...
export const aiMessagesHistory = pgTable("ai_messages_history", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
	userId: text("user_id").notNull(),
...
(table) => {
	return {
		idxAiMessagesHistoryUserId: index("idx_ai_messages_history_user_id").on(table.userId),
	}
});
...
(table) => {
	return {
		portfolioBrandUserIdUnique: unique("portfolio_brand_user_id_unique").on(table.userId),
	}
});
...
export const budgets = pgTable("budgets", {
	...
	averagePricePerM2: numeric("average_price_per_m2", { precision: 10, scale:  2 }),
	...
	complexityPercentage: numeric("complexity_percentage", { precision: 5, scale:  2 }),
	deliveryTimePercentage: numeric("delivery_time_percentage", { precision: 5, scale:  2 }),
	deliveryTimeDays: integer("delivery_time_days"),
});

export const portfolioImages = pgTable("portfolio_images", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
	projectId: uuid("project_id").references(() => portfolioProjects.id, { onDelete: "cascade" } ),
...
export const budgetActivities = pgTable("budget_activities", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
	phaseId: uuid("phase_id").references(() => budgetPhases.id, { onDelete: "cascade" } ),
	segmentId: uuid("segment_id").references(() => budgetSegments.id, { onDelete: "cascade" } ),
	name: text("name").notNull(),
	description: text("description"),
